...
int LseekFile(int fd,int size,int from)
{
	...
	if((UFDTArr[fd].ptrfiletable->mode == READ)||(UFDTArr[fd].ptrfiletable->mode == READ+WRITE))
	{
		if(from == CURRENT)
		{
			if(((UFDTArr[fd].ptrfiletable->readoffset)+size) > UFDTArr[fd].ptrfiletable->FileActualSize)
				return -1;
...
int main()
{
	...
	while(1)
	{
		...
		if(count == 1)
		{
			...
			{
				CloseAllFile();
				printf("All Fles closed succesfully\n");
				continue;
			}
			...
			{
				printf("Terminating the Marvollous Virtual file system \n");
				break;
			}
			else
			{
		...
		if(count == 2)
		{
			...
			else if(strcmp(command[0], "write") == 0)
			{
				...
				if(ret == -2)
					printf("ERROR: There is no sufficientmemory to write\n");
