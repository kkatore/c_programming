...
void DisplayHelp()
{
	...
	printf("closeall : To list out all files\n");
...
/*int ReadFile(int fd, char *arr, int isize)
 {
     ...
     if(UFDTArr[fd].ptrfiletable -> ptrinode -> permission != READ && UFDTArr[fd].ptrfiletable ->
     ...
     read_size = (UFDTArr[fd].ptrfiletable -> ptrinode -> FileActualSize) - (UFDTArr[fd].ptrfiletable
	 -> readoffset);
	 
	 if(read_size < isize)
	 {
		 strncpy(arr, (UFDTArr[fd].ptrfiletable -> ptrinode -> Buffer) + (UFDTArr[fd].ptrfiletable 
...
int main()
 {
	 ...
	 char command[4][80], str[80], arr[2048];
